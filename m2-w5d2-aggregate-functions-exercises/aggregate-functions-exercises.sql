...
SELECT (gnp*1000000/ population) FROM country WHERE population IS NOT NULL AND population != 0 ORDER BY gnp/population DESC
...
SELECT (population/surfacearea)AS population_density FROM country ORDER BY population_density DESC LIMIT 1
...
SELECT ABS(gnp - gnpold)AS abs_difference, (gnp-gnpold) AS difference, name FROM country 
...
SELECT AVG(population) AS avg_population, countrycode FROM city GROUP BY countrycode ORDER BY avg_population DESC
	
-- 18. The count of cities in each state in the USA, ordered by state name.
-- (45 rows)
SELECT COUNT(name)AS number_of_cities, district FROM city WHERE countrycode ='USA' GROUP BY district ORDER BY district
-- 19. The count of countries on each continent, ordered from highest to lowest.
-- (highest count: 58, "Africa")
SELECT COUNT(name) AS number_of_countries, continent FROM country GROUP BY continent ORDER BY number_of_countries	
...
GROUP BY continent ORDER BY avg_life_expectancy ASC 